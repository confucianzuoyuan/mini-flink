...
public class MiniCluster implements JobExecutorService, AutoCloseableAsync {
	...
	@GuardedBy("lock")
	private void setupDispatcherResourceManagerComponents(Configuration configuration, RpcServiceFactory dispatcherResourceManagreComponentRpcServiceFactory) throws Exception {
	...
	public CompletableFuture<JobSubmissionResult> submitJob(JobGraph jobGraph) {
		...
		final CompletableFuture<Acknowledge> acknowledgeCompletableFuture = blobServerAddressFuture
			.thenCombine(
				...
				(InetSocketAddress ack, DispatcherGateway dispatcherGateway) -> dispatcherGateway.submitJob(jobGraph, rpcTimeout))
